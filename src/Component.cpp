...
void Component::AddError(unsigned short code, const std::string &src, const std::string &msg, bool throw_excp) {
    if (!connection || !memory_manager) {
        return; // Не можем добавить ошибку без менеджеров
    }
    ...
    try {
        ...
        if (source) memory_manager->FreeMemory(reinterpret_cast<void **>(&source));
        if (descr) memory_manager->FreeMemory(reinterpret_cast<void **>(&descr));
        throw;
    }
...
bool Component::ExternalEvent(const std::string &src, const std::string &msg, const std::string &data) {
    if (!connection || !memory_manager) {
        return false; // Не можем отправить событие без менеджеров
    }
    ...
    try {
        ...
        // Освобождаем память в случае исключения
        if (wszSource) memory_manager->FreeMemory(reinterpret_cast<void **>(&wszSource));
        if (wszMessage) memory_manager->FreeMemory(reinterpret_cast<void **>(&wszMessage));
        if (wszData) memory_manager->FreeMemory(reinterpret_cast<void **>(&wszData));
        throw;
    }
}
